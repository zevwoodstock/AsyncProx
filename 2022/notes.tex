...
repo)
\item
\textbf{Theory and Vocabulary Homework} (if a question is not provided,
...
f(x)$ points.
\end{itemize}
One of our main tasks is to program asynchronous algorithms which
involve gradients, projections, and proximity operators. These
algorithms theoretically are proven to work in certain situations;
however, they have not been asynchronously implemented to-date. A
big research goal is to determine
hyperparameter selection strategies for each algorithm, and write
up a comparison of the available algorithms.
Optimization algorithms -- which are the backbone of ML -- arise as
a special case of these algorithms.

Many of the papers provided in the Literature folder involve
...
\item
Initial testing: implement algorithms for $(T_i)_{i\in I}$ being
projections onto simple shapes in 2D (boxes, balls, half-planes,
subspaces). Construct visualizations.
\end{itemize}
\end{itemize}
...
\item
onto simple shapes in 2D (boxes, balls, half-planes, subspaces).
ow-rank + sparse decomposition -- smooth, although there are
...
Mathieu Besan\c{c}on (research)
Heike Balliunet (General admin questions)
\end{document}
